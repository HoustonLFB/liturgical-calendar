#!/usr/bin/env python3
"""
Script 2 amélioré : Préserve TOUS les commentaires et ne modifie QUE les labels.
"""
import os
import sys
import re
    ...
    for block in blocks:
        ...
        for line in block.split('\n'):
            ...
            if line.startswith('FICHIER:'):
                ...
                # On accepte désormais une valeur vide
                current['label_fr'] = line.replace('LABEL_FR:', '').strip()
        # Plus besoin de tester si label_fr est non vide
        if 'file' in current and 'label_fr' in current:
            translations[current['file']][current['index']] = current['label_fr']
    ...
    for i, line in enumerate(lines):
        ...
        if in_history and current_history_index in translations_dict:
            if re.match(r'\s*label:', line):
                ...
                new_line = f'{indent}label: "{translations_dict[current_history_index]}"'
                
                if new_line != old_label:
                    lines[i] = new_line
                    modified = True
                    print(f"  Index {current_history_index}:")
                    print(f"    Ancien: {old_label.strip()}")
                    print(f"    Nouveau: {new_line.strip()}")
...
def main():
    ...
    print(f"🔍 {total_files} fichiers à traiter, {total_labels} labels à modifier\n")
    ...
    print(f"📊 {total_labels} labels traités")
